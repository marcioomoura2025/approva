...
- **Marcações** — abas: marcadas p/ revisão · favoritas · erros & chutes (montado automaticamente). Cada aba tem busca e filtro por matéria; no caderno de erros é possível marcar **“Já estudei esta”** (a questão sai da lista e volta sozinha se for errada de novo) ou **limpar o caderno** inteiro.
...
- **Modelos de simulado** — salve uma montagem completa (matérias e quantidades, tempo, modo de correção) com um nome e reutilize com um clique. Aparecem no topo de *Novo simulado*, com resumo em uma linha, contador de uso, e ações de ajustar, renomear e excluir. Cada usuário só enxerga os seus.
- **Tema Dia / Noite** — alternador no rodapé do menu. Na primeira visita segue a preferência do sistema; depois vale a escolha do usuário (guardada no navegador). O menu lateral permanece navy nos dois modos, e a impressão sai sempre clara.
- **Contagem regressiva** — o Painel pergunta uma vez se há concurso à vista; marcando a data, um card mostra quantos dias faltam junto do que há para fazer hoje (revisões pendentes). Quem responde “agora não” não é perguntado de novo, mas mantém um acesso discreto para marcar depois. Aceita mais de um concurso, usa o dia local do usuário e muda de tom quando a data passa.
- **Mapa de constância** — no Painel, um mapa de calor dos últimos ~3 meses (uma célula por dia, intensidade pelo volume de questões), com dias estudados no mês e melhor sequência. Respeita o fuso do navegador, então estudo noturno conta no dia correto.
- **Resumo do banco no Painel** — faixa com o total de questões, provas completas e matérias disponíveis.
